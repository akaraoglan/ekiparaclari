...
class IhracKayitliFinalTest(unittest.TestCase):
    ...
    def test_all_differences_and_total_are_reconciled(self):
        with _flat_workspace(self.TEST_ROOT) as (folder, prefix):
            ...
            for row, expected in zip(range(5, 8), expected_bases):
                ...
                self.assertEqual(float(kdv), result.cell(row, 16).value)
                self.assertEqual(0, result.cell(row, 17).value)
                actual_total += actual

            total_kdv = Decimal(str(result["L8"].value)).quantize(Decimal("0.01"))
            self.assertEqual(total_kdv / Decimal("0.20"), actual_total)
            self.assertEqual("TOPLAM", result["K8"].value)
            self.assertIsNone(result["P8"].value)
            self.assertIsNone(result["Q8"].value)
            workbook.close()
    ...
    def test_web_route_downloads_final_workbook(self):
        with _flat_workspace(self.TEST_ROOT) as (folder, prefix):
            ...
            self.assertEqual(272882.36, downloaded["Sonuc"]["P7"].value)
            self.assertEqual(0, downloaded["Sonuc"]["Q7"].value)
            downloaded.close()
            response.close()
